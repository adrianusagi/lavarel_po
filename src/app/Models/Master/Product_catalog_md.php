<?php 
namespace App\Models\Master;

use Illuminate\Database\Eloquent\Model;
use Illuminate\Support\Facades\DB;

class Product_catalog_md extends Model
{
    public static function get_query()
    {
        $sql = "SELECT
                prd.id AS `id`,
                Code,
                Name,
                cat.label AS `Category`,
                last_price AS `Last Purchase Price`,
                NULL AS `[[Menus]]`
            FROM
                ".DB_P2P."tb_p2p_rf_product_catalog prd
            LEFT JOIN ".DB_P2P."tb_p2p_rf_options cat ON prd.category = cat.id
            AND cat.category = 'product_category'
            ORDER BY
                Name";
        return $sql;
    }

    public static function get_all()
    {
        $sql = self::get_query();
        return DB::select($sql);
    }

    public static function get_one($id)
    {
        return DB::table(DB_P2P.'tb_p2p_rf_product_catalog')->where('id', $id)->first();
    }

    public static function get_one_formatted($id)
    {
        $sql = "SELECT
                Code,
                Name,
                cat.label AS `Category`,
                default_uom AS `Default Unit of Measure`,
                last_price AS `Last Purchase Price`
            FROM
                ".DB_P2P."tb_p2p_rf_product_catalog prd
            LEFT JOIN ".DB_P2P."tb_p2p_rf_options cat ON prd.category = cat.id
            WHERE
                prd.id = ?";
        $dbres = DB::select($sql, [$id]);
        if(is_array($dbres) && count($dbres) == 1) return (array) $dbres[0];
        else return false;
    }

    public static function get_options()
    {
        $sql = "SELECT
                prd.id,
                cat.label AS `group`,
                CONCAT(`code`,' | ', `name`) AS `caption`
            FROM
                ".DB_P2P."tb_p2p_rf_product_catalog prd
            LEFT JOIN ".DB_P2P."tb_p2p_rf_options cat ON prd.category = cat.id
            AND cat.category = 'product_category'
            ORDER BY
                `group`,
                `caption`";
        return DB::select($sql);
    }

    public static function get_category_options(){
        $sql = "SELECT
                id,
                label AS `caption`
            FROM
                ".DB_P2P."tb_p2p_rf_options
            WHERE
                category = 'product_category'
            AND is_active = 'YES'
            ORDER BY
                `ordering` ASC,
                `caption` ASC";
        return DB::select($sql);
    }

    public static function set_data($data)
    {
        $table = DB_P2P.'tb_p2p_rf_product_catalog';

        if(empty($data['form_data'])) return array('isOk' => false, 'msg' => 'Invalid parameters');

        $is = null;
        $msg = null;

        $form_data = $data['form_data'];
        if(empty($form_data['id'])){
            $set = insert_log($form_data);
            
            $is = DB::table($table )->insert($set);
            if(!$is) $msg = 'Failed to inserting data to database';
        } else {
            $set = update_log($form_data);
            $is = DB::table($table )->where('id', $form_data['id'])->update($set);
            if(!$is) $msg = 'Failed to updating data to database';
        }

        return array('isOk' => $is, 'msg' => $msg);
    }

    public static function set_delete($id)
    {
        $is = DB::table(DB_P2P.'tb_p2p_rf_product_catalog')->where('id', $id)->delete();
        return array('isOk' => $is, 'msg' => '');
    }
}