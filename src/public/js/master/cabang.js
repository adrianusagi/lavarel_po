var cabang = {
    ...
    view: function(ins){
        ...
        _this.mdl_view.find('.modal-title').html('Vew Cabang' + params['Name']);
    ...
    form_input: function(ins = null){
        ...
        let params = {};
        if(ins == null){
            _this.mdl_input.find('.modal-title').html('Add New Cabang');
        } else {
            params = JSON.parse(atob(urldecode($(ins).closest('.dropdown').attr('data-params'))));
            _this.mdl_input.find('.modal-title').html('Edit Cabang ' + params['Name']);
        }
...
$(cabang._mdl_input).on('click', 'button[data-aksi="mdl_save"]', function(){
    cabang.form_save();
})
...
$('body').on('click', 'a[data-aksi="dtrow_delete"]', function(){
    ...
    bootbox.confirm('Are you sure to delete ' + cabang.title + ' named <b>' + params['Name'] + '</b> ?', function (result){ 
        cabang.delete(params);
    });
})