var supplier = {
    ...
    view: function(ins){
        ...
        _this.mdl_view.find('.modal-title').html('Vew Supplier ' + params['Name']);
    ...
    form_input: function(ins = null){
        ...
        let params = {};
        if(ins == null){
            _this.mdl_input.find('.modal-title').html('Add New Supplier');
        } else {
            params = JSON.parse(atob(urldecode($(ins).closest('.dropdown').attr('data-params'))));
            _this.mdl_input.find('.modal-title').html('Edit Supplier ' + params['Name']);
        }
        ...
        $.get(admin.page_url + '/form', params, function(respon){
            ...
            _this.form = _this.mdl_input.find('form#form_supplier').my_plugins().form();
...
$(supplier._mdl_input).on('click', 'button[data-aksi="mdl_save"]', function(){
    supplier.form_save();
})
...
$('body').on('click', 'a[data-aksi="dtrow_delete"]', function(){
    ...
    bootbox.confirm('Are you sure to delete ' + supplier.title + ' named <b>' + params['Name'] + '</b> ?', function (result){ 
        if(result) supplier.delete(params);
    });
})